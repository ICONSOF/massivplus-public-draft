...
Därför går DPP:ns relation till MASSIV+ på koldioxidsidan genom MASSIV+:s relation till [produktstandarderna](produktstandarder.md). Ett MASSIV+-nodvärde är en annan storhet än en PEF-PCF: det gäller en nod och inte en produkt, det täcker det uppströms ledet fram till grinden, och det allokerar och kvalitetsmärker enligt egen logik (massa, energi eller värde; Coverage = A/(A+U)) i stället för PEF:s allokerings- och DQR-regler. DPP är förankrat i produktartefakten - en ny produkt får ett nytt pass som refererar uppströms pass - medan MASSIV+ är förankrat i organisationen och affärsrelationen (se [koordinatbytet](../../fordjupningar/koordinatbytet.md)). Det är därför ett MASSIV+-nodvärde är *underlag* till passets koldioxidfält, inte fältet självt: för en reglerad produktgrupp som batterier är MASSIV+-data underlaget som den föreskrivna avtrycksberäkningen förbrukar.
